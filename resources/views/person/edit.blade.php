...
@section('content')
    ...
    <div class="row">
        <div class="col-xs-12">
            <form id="edit_form" class="form-horizontal" role="form" action="/person/{{ $person['id'] }}" method="post">
                {{ csrf_field() }}
                {{ method_field('PATCH') }}
                <input type="hidden" name="id" value="{{ $person['id'] }}">
                @include('person.partials.form', ['person' => $person, 'submit_button' => Lang::get('form.save_change')])
            </form>
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <form class="form-horizontal" role="form" id="changePassword_form">
                    <div class="modal-body">
                        <input type="hidden" id="user_id" name="user_id" value="{{ $person['id'] }}">
                        <div class="form-group">
                            ...
                            <div class="col-xs-6">
                                <h4 style="margin-top:7px;">{{ $person['username'] }}</h4>
                            </div>
                        </div>
